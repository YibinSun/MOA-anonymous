 ...
 */
public class DEMS_OAUE extends AbstractClassifier implements MultiClassClassifier {
	...
	public IntOption memberCountOption = new IntOption("memberCount", 'n',
			"The maximum number of classifiers in an ensemble.", 100, 1, Integer.MAX_VALUE);
	 ...
	 * Determines whether additional information should be sent to the output.
	 */
	public FlagOption linearOption = new FlagOption("linearFunction", 'x', "When checked the algorithm uses a linear weighting function.");

	// Yibin New
	public IntOption kValueOption = new IntOption("kValues", 'k', "K values", 5, 1, this.memberCountOption.getValue());
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (this.selfOptimisingOption.isSet()) {
			...
			for (int i = this.infos.size() - 1; i >= 0; i--) {
				...
				if (combinedVotes.maxIndex() == inst.classValue())
					this.performances[this.ensemble.length - i - 1]++;
			}
        ...
        if (this.processedInstances % this.windowSize == 0) {
			...
			this.candidate.evaluator.addResult(new InstanceExample(inst), this.candidate.classifier.getVotesForInstance(inst));
    	...
    	for (int i = 0; i < this.ensemble.length; i++) {
			this.ensemble[i].evaluator.addResult(new InstanceExample(inst), this.ensemble[i].classifier.getVotesForInstance(inst));
	...
	public double[] getVotesForInstance(Instance inst) {
		...
		for (int i = 0; i < this.ensemble.length; i++)
			this.infos.add(new SortingInformation(this.ensemble[i].evaluator.getTotalWeightObserved() == 0 ? 0 : this.ensemble[i].evaluator.getFractionCorrectlyClassified(), this.ensemble[i].classifier.getVotesForInstance(inst), i));
		...
		else this.bestK = Math.min(this.kValueOption.getValue(), this.infos.size());

		if (this.trainingWeightSeenByModel > 0.0) {
			for (int i = 0; i < this.bestK && i < this.ensemble.length; i++) {
				int index = this.infos.get(this.ensemble.length - i - 1).getClassifierIndex();
				if (this.weights[index][0] > 0.0){
					DoubleVector vote = new DoubleVector(this.ensemble[(int) this.weights[index][1]].classifier.getVotesForInstance(inst));
					if (vote.sumOfValues() > 0.0) {
						vote.normalize();
						// scale weight and prevent overflow
						vote.scaleValues(this.weights[index][0] / (1.0 * this.ensemble.length + 1.0));
...
//			}
    ...
    protected double computeWeight(int i, Instance example) {
        ...
        if(linearOption.isSet())
        {
        	return Math.max(mse_r - mse_it, Double.MIN_VALUE);
        }
        else
        {
	...
	protected void addToStored(ClassifierWithMemory newClassifier, double newClassifiersWeight) {
		...
		// Add performance only when self adapting
		if (this.selfOptimisingOption.isSet()) {
		int[] performances = new int[newStored.length];
			System.arraycopy(this.performances, 0, performances, 0, this.performances.length);
			if (this.performances.length > 0)
				performances[performances.length - 1] = Utils.sum(this.performances) / this.performances.length;
