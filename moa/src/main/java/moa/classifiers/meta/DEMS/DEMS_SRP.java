 ...
 * @version $Revision: 1 $
 */
public class DEMS_SRP extends AbstractClassifier implements MultiClassClassifier,
        CapabilitiesHandler {
    ...
    public void trainOnInstanceImpl(Instance instance) {
        if (this.selfOptimisingOption.isSet() && this.instancesSeen == 0) this.performances = new int[this.ensemble.length];
        ++this.instancesSeen;
        ...
        if (this.selfOptimisingOption.isSet()) {
            ...
            for (int i = this.infos.size() - 1; i >= 0; i--) {
                SortingInformation s = this.infos.get(i);
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        for(int i = 0; i < this.ensemble.length; i++) {
            if (this.ensemble[i].classifier instanceof HoeffdingTree) {
                ...
                this.infos.add(new SortingInformation(this.ensemble[i].evaluator.getTotalWeightObserved() == 0 ? 0 : this.ensemble[i].evaluator.getFractionCorrectlyClassified(), ht.getTreeRoot() == null ? new double[]{0} : ht.getTreeRoot().filterInstanceToLeaf(testInstance, null, -1).node == null ? new double[]{0} : ht.getTreeRoot().filterInstanceToLeaf(testInstance, null, -1).node.getObservedClassDistribution(), i));
            }
//            this.infos.add(new SortingInformation(this.ensemble[i].evaluator.getTotalWeightObserved() == 0 ? 0 : this.ensemble[i].evaluator.getFractionCorrectlyClassified(), this.ensemble[i].getVotesForInstance(testInstance),i));
        ...
        for (int i = 0; i < this.bestK; i++) {
            SortingInformation s = this.infos.get(this.ensemble.length - 1 - i);
    ...
    protected void initEnsemble(Instance instance) {
        ...
        if (this.trainingMethodOption.getChosenIndex() != DEMS_SRP.TRAIN_RESAMPLING) {
            ...
            switch (this.subspaceModeOption.getChosenIndex()) {
                case DEMS_SRP.FEATURES_SQRT:
                    k = (int) Math.round(Math.sqrt(n)) + 1;
                    break;
                case DEMS_SRP.FEATURES_SQRT_INV:
                    k = n - (int) Math.round(Math.sqrt(n) + 1);
                    break;
                case DEMS_SRP.FEATURES_PERCENT:
            ...
            if (this.trainingMethodOption.getChosenIndex() == DEMS_SRP.TRAIN_RANDOM_SUBSPACES ||
                    this.trainingMethodOption.getChosenIndex() == DEMS_SRP.TRAIN_RANDOM_PATCHES) {
                if (k != 0 && k < n) {
                    ...
                    if (n <= 20 || k < 2) {
                        ...
                        this.subspaces = DEMS_SRP.allKCombinations(k, n);
                    ...
                    // subspaces without worrying about repetitions.
                        ...
                        this.subspaces = DEMS_SRP.localRandomKCombinations(k, n,
                ...
                // k == 0 or k > n (subspace size greater than the total number of features), then default to resampling
                    ...
                    this.trainingMethodOption.setChosenIndex(DEMS_SRP.TRAIN_RESAMPLING);
                }
            }
        }
        ...
        for (int i = 0; i < ensembleSize; ++i) {
            switch (this.trainingMethodOption.getChosenIndex()) {
                case DEMS_SRP.TRAIN_RESAMPLING:
                    this.ensemble[i] = new StreamingRandomPatchesClassifier(
                            ...
                            false);
                    break;
                case DEMS_SRP.TRAIN_RANDOM_SUBSPACES:
                case DEMS_SRP.TRAIN_RANDOM_PATCHES:
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == DEMS_SRP.class)
