 ...
 * node splits (See moa.classifiers.trees.ESCARFTree.java); (3) drift
 ...
 *
 * <p>Parameters:</p> <ul>
 * <li>-l : Classiﬁer to train. Must be set to ESCARFTree</li>
 ...
 * @version $Revision: 1 $
 */
public class DEMS_ARF extends AbstractClassifier implements MultiClassClassifier,
        CapabilitiesHandler {
    ...
    private List<SortingInformation> informations;
    ...
    public void resetLearningImpl() {
        ...
        if (numberOfJobs != DEMS_ARF.SINGLE_THREAD && numberOfJobs != 1)
            this.executor = Executors.newFixedThreadPool(numberOfJobs);

        if (this.informations == null) this.informations = new ArrayList<SortingInformation>();
    }
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        if (this.selfOptimisingOption.isSet()) {
            ...
            for (int i = this.informations.size() - 1; i >= 0; i--) {
                ARFBaseLearner arfBaseLearner = this.ensemble[this.informations.get(i).getClassifierIndex()];
...
//                if (combinedVotes.maxIndex() == instance.classValue()){
//                    if (this.windowedPerformanceOption.isSet()){
//                        if (this.wPerformances == null) for (int n = 0; n < this.ensembleSizeOption.getValue(); n++) this.wPerformances.add(new ArrayList<>());
//                        this.wPerformances.get(i).add(1.0);
//                        if (this.wPerformances.get(i).size() > this.windowSizeOption.getValue()) this.wPerformances.remove(0);
//                    }else{
//                        this.performances[this.ensembleSizeOption.getValue() - i - 1]++;
//                    }
//                }
...
//            this.kBest = getMaxIndex(this.performances) + 1;
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        if (this.selfOptimisingOption.isSet() && this.performances == null)
            this.performances = new double[this.ensembleSizeOption.getValue()];
        this.informations = new ArrayList<>();

        for (int i = 0; i < this.ensemble.length; i++) {
            this.informations.add(new SortingInformation(this.ensemble[i].evaluator.getTotalWeightObserved() == 0 ? 0 : this.ensemble[i].evaluator.getFractionCorrectlyClassified(), this.ensemble[i].classifier.getTreeRoot() == null ? new double[]{0} : this.ensemble[i].classifier.getTreeRoot().filterInstanceToLeaf(instance, null, -1).node == null ? new double[]{0} : this.ensemble[i].classifier.getTreeRoot().filterInstanceToLeaf(instance, null, -1).node.getObservedClassDistribution(), i));
        }
        this.informations = this.informations.stream().sorted(Comparator.comparing(SortingInformation::getMargin_TreeAcc)).collect(Collectors.toList());

        if (this.selfOptimisingOption.isSet()) System.out.println(this.kBest);
        else this.kBest = this.kValueOption.getValue();
...
//                trees += treeIndex + ", ";
                ARFBaseLearner arfBaseLearner = this.ensemble[treeIndex];
...
//            treeLearner.subspaceSizeOption.setValue(this.subspaceSize);
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == DEMS_ARF.class)
    ...
    public Classifier[] getSublearners() {
        /* Extracts the reference to the ESCARFTree object from within the ensemble of ARFBaseLearner's */
